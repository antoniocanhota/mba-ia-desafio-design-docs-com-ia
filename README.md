...
Este desafio parte da transcrição literal de uma reunião técnica em que a equipe de um Order Management System (OMS) já existente decide construir um Sistema de Webhooks de Notificação de Pedidos, sem produzir nenhum artefato de documentação além da própria gravação da call (`TRANSCRICAO.md`). A tarefa é usar IA como ferramenta de produção para transformar essa transcrição, somada ao código real do OMS, num pacote completo de design docs (PRD, RFC, FDD, ADRs e Tracker de rastreabilidade) acionável o suficiente para o time de engenharia começar a implementar a feature, sem inventar requisito, decisão ou restrição que não tenha origem identificável na reunião ou no código.

Meu papel não foi pedir "gere um PRD a partir dessa transcrição" e aceitar o resultado de primeira, mas conduzir cada documento com prompts dirigidos, comandos de projeto reutilizáveis e verificação cruzada constante contra a transcrição, o código e os critérios de aceite do enunciado original (`README.original.md`) — inclusive auditando a própria saída da IA depois de pronta, para encontrar e corrigir divergências de formato e itens sem rastreabilidade real.
...
6. **Consolidação do PRD.** Usei o prompt de entrevista de PRD disponibilizado no curso, salvo como comando de projeto em [`.claude/commands/geracao-de-prd.md`](.claude/commands/geracao-de-prd.md) e invocável como `/geracao-de-prd`, seguindo a "Ordem de execução sugerida" do enunciado original (PRD por último entre os documentos grandes). Antes de perguntar qualquer coisa, ele leu a transcrição inteira, o RFC, as 8 ADRs, o FDD e o Tracker, e me apresentou etapa por etapa (contexto, problema, escopo, requisitos funcionais espelhando os 8 contratos já fechados no FDD, decisões e riscos) o que já estava determinado por essas fontes, pra eu só confirmar. As perguntas reais ficaram para o que nenhum documento cobria: uma segunda métrica de sucesso além da latência (propus redução de chamadas a `GET /orders`, com meta conservadora sugerida por mim), a meta de disponibilidade (não discutida na reunião, assumi 99.9% como hipótese), e a estratégia de testes (também não discutida, propus TDD para a lógica crítica de HMAC/backoff mais QA manual guiado por roteiro). Ao final, escreveu `docs/PRD.md` com os 8 requisitos funcionais, e atualizou `docs/TRACKER.md` com 41 novas linhas.
7. **Auditoria contra o enunciado original.** Depois do PRD pronto, pedi para reler `README.original.md` e verificar `docs/PRD.md` e `docs/TRACKER.md` contra a lista de critérios de aceite item por item, em vez de assumir que estava tudo certo. A auditoria confirmou todos os critérios do PRD, mas encontrou uma divergência real no Tracker: 5 linhas usavam `Fonte = ENTREVISTA` para itens que só vieram da minha decisão durante a entrevista (meta de redução de polling, disponibilidade, observabilidade, acessibilidade, ausência de infra nova), um valor que a skill de PRD aceita mas que o enunciado original não prevê — ele define a coluna `Fonte` como tendo só `TRANSCRICAO` ou `CODIGO`. Removi essas 5 linhas do Tracker (os fatos continuam no PRD, marcados como hipótese no próprio texto) e renumerei os IDs restantes para eliminar os gaps deixados pela remoção.
...
- **`docs/PRD.md`** — visão de produto/negócio: problema, público-alvo, 2 objetivos com métrica e meta, escopo incluso/fora, 8 requisitos funcionais, requisitos não funcionais, decisões e trade-offs, dependências, riscos, critérios de aceitação e estratégia de testes, consolidando RFC, ADRs e FDD já fechados.
- **`docs/TRACKER.md`** — tabela de rastreabilidade cruzada: liga cada item documentado (ADRs, RFC, FDD e PRD) de volta à `TRANSCRICAO.md` ou a um caminho real em `src/`/`prisma/`, com `Fonte` restrita a `TRANSCRICAO` ou `CODIGO`, como exige o enunciado.
