...
O trabalho não saiu limpo de primeira. A parte mais instrutiva foi um erro de rota **meu**, logo no começo, e depois uma sequência de ajustes finos nos prompts e no Tracker — quase todos motivados por problemas concretos encontrados **ao revisar** a saída, não ao rodar às cegas. Abaixo, por fase.

### Fase de ADRs

**1. O desvio de rota inicial (interpretação minha do enunciado).** Comecei entendendo o desafio como "documentar a arquitetura do app existente" e rodei o plugin genérico `adrs-management`/`project-analizer` sobre o código. Ele fez exatamente o que pedi: mapeou `src/` (`mapping.md`, `potential-adrs-index.md`) e gerou ADRs do **OMS que já existe** — ciclo de vida transacional de pedidos, framework em camadas com DI manual, versionamento REST, JWT+RBAC, MySQL, Prisma. Só ao revisar percebi que o desafio pede as ADRs das **decisões tomadas na reunião sobre a feature de webhooks**, não uma engenharia reversa do código atual. Um sinal claro do desalinhamento veio da própria saída: os arquivos vinham cheios de placeholders `[ENTRADA NECESSÁRIA: ...]` (qual o throughput esperado, se a numeração sem-lacunas é regra rígida de negócio) — perguntas que só os desenvolvedores originais poderiam responder, ou seja, a IA estava documentando um sistema que não era o objeto do desafio. Descartei todo o material gerado (a árvore `generated/`, o mapping, o índice, uma ADR errada, o README da pasta) e limpei o `CLAUDE.md` das referências obsoletas. Foi esse descarte que motivou a escrever um comando próprio, o `/adr-from-meeting`, que extrai decisão **da transcrição** em vez de mapear código.

O `/adr-from-meeting` então passou por **4 refinamentos**, cada um por um problema concreto de revisão:

**2. Alternativas rejeitadas se perdendo.** Itens como "Redis Streams" (rejeitado em favor do outbox em MySQL) ou "3 tentativas" (rejeitado em favor de 5) caíam direto na lista de excluídos, em vez de alimentar "Opções Consideradas"/"Prós e Contras" do ADR da decisão correspondente. Adicionei uma etapa que associa cada alternativa rejeitada à decisão candidata a que ela pertence.

**3. Risco de alucinação sem verificação.** Não havia passo que checasse se o resumo apresentado batia com a transcrição. Adicionei uma verificação cruzada obrigatória antes de exibir qualquer tabela, com limite de 2 iterações de correção por item e um marcador "⚠️ não confirmado" para divergências que não fecham.

**4. Decisão real ficando de fora por falta de "debate explícito".** A decisão de "reaproveitar os padrões já existentes do projeto" (reafirmada por Larissa no fechamento, `[09:48]`) não aparecia como candidata: o prompt exigia implicitamente um debate "opção A vs. opção B", e decisões de convenção/reuso não têm esse formato. Removi o viés e acrescentei uma checklist de cobertura que usa a fala de resumo/fechamento da reunião como lista de conferência.

**5. Tracker apontando pro momento errado.** Quando uma decisão era discutida no meio e só reafirmada no resumo final, o tracker citava a discussão inicial em vez da confirmação. Ajustei a regra de preenchimento do `docs/TRACKER.md` para sempre priorizar a fala que efetivamente fecha a decisão.

### Fase de RFC

**6. Tracker abaixo de 70% (primeira ocorrência).** Depois de gerar o RFC, uma auditoria contra os critérios de aceite mostrou a proporção de linhas com `Fonte = TRANSCRICAO` em 68% — abaixo do mínimo de 70%. Em vez de inflar a tabela, acrescentei 3 linhas que já deveriam existir por conteúdo e eram rastreáveis a falas **já citadas nas ADRs**: o precedente real de vazamento de secret (`[09:22] Diego`), o de indisponibilidade de 2h por manutenção (`[09:16] Diego`) e o requisito de latência do cliente (`[09:02] Marcos`). Subiu para 71%.

### Fase de FDD

**7. Prompt de FDD amarrado demais à feature.** A primeira versão do comando de geração de FDD tinha exemplos colados à feature de webhooks (timestamps, `ADR-008`, `publishWebhookEvent`, `WEBHOOK_*`). Generalizei para servir a qualquer feature da aplicação, ampliei a Fase 0 para ler `docs/` recursivamente (incluindo os relatórios de análise) e — ponto importante contra alucinação — instruí explicitamente a **ignorar resumos derivados** (`docs/others/ata-*.md`) em favor de `TRANSCRICAO.md` como fonte primária de citação `[hh:mm] Nome`, para a IA não citar o próprio resumo dela como se fosse a fonte.

**8. Hipótese de rota sem fonte, entrando como se fosse fato.** Ao especificar os contratos públicos, a IA misturou caminhos concretos inferidos por convenção (`/api/v1/webhooks/...`) com os dois que a reunião realmente cita (`GET /webhooks/:id/deliveries`, `POST /admin/webhooks/dead-letter/:id/replay`), sem distinguir fala de inferência. Pedi para remover as hipóteses da narrativa de fluxo e resolvi a lacuna com uma pergunta direta, em vez de deixar a IA assumir em silêncio — mesmo padrão aplicado ao armazenamento da secret e ao tracing na observabilidade.

**9. Tracker abaixo de 70% (segunda ocorrência).** A seção "Integração com o sistema existente" adicionou várias linhas `CODIGO` de uma vez e a proporção `TRANSCRICAO` caiu para 68,5%. Novamente sem inflar: acrescentei linhas que já deveriam existir por conteúdo (os fluxos de "processamento pelo worker", "retry" e "DLQ" como itens próprios do FDD, e a restrição de escopo outbound-only), o que de quebra fechou a cobertura dos "quatro momentos de fluxo" exigidos pelo enunciado.

### Fase de PRD

**10. Valor de `Fonte` fora do formato exigido.** Ao gerar o PRD, a skill de entrevista registrou no Tracker `Fonte = ENTREVISTA` para 5 itens que só existiam porque eu os decidi durante a entrevista (redução de chamadas a `GET /orders`, disponibilidade de 99.9%, observabilidade, acessibilidade, ausência de infra nova). Uma auditoria contra `README.original.md` mostrou que a coluna `Fonte` só admite `TRANSCRICAO` ou `CODIGO` — `ENTREVISTA` era convenção da skill, não do desafio. Removi as 5 linhas (os fatos seguem no PRD, marcados como hipótese) e renumerei os IDs para não deixar gaps.

**11. Seção obrigatória rebaixada a rótulo pelo template do comando.** Ao conferir o PRD contra o requisito 1 do enunciado, notei que "Público-alvo e cenários de uso" — listada lá como uma das seções obrigatórias — não aparecia como seção própria: o "Esqueleto de PRD" embutido no comando `/geracao-de-prd` fixava "Público-alvo" e "Cenários de uso chave" como rótulos de texto puro dentro de "Contexto e problema", e a geração seguiu o template à risca. Não foi alucinação da IA: o próprio template do prompt já nascia divergente do enunciado, então o furo se reproduziria a cada nova geração. Corrigi na raiz, promovendo o bloco a heading próprio (`### Público-alvo e cenários de uso`) tanto no PRD quanto no esqueleto do comando, e acrescentei uma checagem de consistência ao comando exigindo que toda seção obrigatória tenha heading próprio.

### Consistência final

**12. Varredura de fechamento contra a transcrição e o enunciado.** Com todos os documentos prontos, um último passo de auditoria pegou o que tinha escapado: duas citações incorretas no Tracker (T018, T089), o placeholder "diversos" no lugar de nomes reais de participantes (T061, T077, T081), a data ausente nas 8 ADRs, e a falta de anotação, na tabela de contratos do FDD, de quais rotas vêm literalmente da reunião e quais são inferidas por convenção. Todos corrigidos.

**13. Contradição entre ADR e FDD sobre a configuração do logger.** Uma revisão de coerência cruzada, lendo os seis documentos contra a transcrição, encontrou uma contradição real: o ADR-008 afirmava que *"não são necessárias mudanças na configuração do logger"*, enquanto o FDD adiciona `*.secret` e `*.signature` aos `redactPaths` do Pino para a secret nunca vazar em log. A raiz era o ADR ter generalizado demais a fala do Bruno (`[09:29]`: "não vamos botar nada novo", que era sobre não introduzir uma biblioteca/componente novo de log) para "nenhuma mudança de config". Como os ADRs foram gerados **antes** do FDD e o endurecimento de redação do FDD é correto (motivado pelo incidente real de secret vazada em log, `[09:22]` Diego), o documento a corrigir era o ADR — não o FDD. Ajustei a consequência do ADR-008 para decidir o *reuso do componente* Pino, reconhecendo o ajuste de `redactPaths` como refinamento de implementação do FDD, não uma decisão da ADR. Foi a disciplina de altitude na prática: a ADR fecha a decisão, o FDD refina o detalhe, e quando os dois divergem, costuma ser a camada mais alta que extrapolou.

**Contagem.** Foram cerca de **13 iterações principais** ao longo do pacote: 1 correção de rota + 4 refinamentos no comando de ADRs, 1 no RFC, 3 no FDD, 2 no PRD e 2 de consistência final entre documentos. Nenhuma delas veio de rodar e aceitar — todas vieram de revisar criticamente a saída contra a transcrição, o código e os critérios de aceite.
